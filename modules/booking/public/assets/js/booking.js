...
async function loadSlots(clearMessage = true) {
    ...
    try {
        ...
        slotGrid.innerHTML = makeSlots().map(time => {
            ...
            return `<button type="button" class="slot ${disabled ? 'disabled' : ''}" data-time="${time}" ${disabled ? 'disabled' : ''}>${time}</button>`;
        }).join('');
...
async function bookSlot(time) {
    ...
    const selectedButton = document.querySelector(`.slot[data-time="${time}"]`);
    selectedButton.classList.add('selected');

    bookingMessage.innerHTML = '<div class="message">Placing your booking...</div>';

    const payload = {
        listing_id: Number(listingIdInput.value),
        tenant_id: Number(tenantIdInput.value),
    ...
    try {
        ...
        if (response.status === 409) {
            bookingMessage.innerHTML = `<div class="message error">Sorry, this slot is already booked. Please choose another time.</div>`;
            await loadSlots(false);
            return;
        }

        if (!data.success) {
            bookingMessage.innerHTML = `<div class="message error">${data.message || 'Booking failed. Please try again.'}</div>`;
            return;
        }
        ...
        bookingMessage.innerHTML = '<div class="message error">Something went wrong. Please try again.</div>';
    }
}
